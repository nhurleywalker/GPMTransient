...
mwax_db2fits -k "2${CHAN}${CHAN}2" --destination-path=${4} --health-netiface=lo --health-ip=224.0.2.2 --health-port=8005 > db2fits${CHAN}.log 2>&1 &
...
cd ${infile}
#for CHAN in $(seq $startChan 1 $endChan); do 
# $(ls *_${CHAN}.sub | sed 's/\(.*\)/dada_diskdb -k 1'${CHAN}${CHAN}'1 -f \1 \&\&/g' | tr '\n' ' ' | sed 's/\&\s$//');
#done
