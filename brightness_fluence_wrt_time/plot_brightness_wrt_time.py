...
# Use the spectral index from the previous source (for now)
alpha = -1.16

# Previously found distance to be 5.2 kpc
dist = 5.2 #kpc

# Previously determined DM
DM = 284
...
files = sorted(glob("../dynspec/1?????????_dyn_dynamic_spectrum.csv"))
...
for f in files:
    ...
    freqlo = np.loadtxt(f.replace("_dyn_dynamic_spectrum.csv", "_frequencies.csv"))[0]/1.e6
    
    # Load a "raw" dynamic spectrum from a CSV file
    dynspec = dd.Dynspec(f, sample_time=0.5, freqlo=freqlo, bw=0.64, time_offset=obsid, transpose=True)
    # Dedisperse it, using an arbitrary reference frequency
    dynspec.dedisperse(DM, freq_ref=154.)
    # Dedisperse it, using an arbitrary reference frequency
    # ...the dedispersed spectrum is now in dynspec.dynspec (NumPy array)
    vals = np.average(dynspec.dynspec, axis=0)
...
# Central frequency is freqlo + 15MHz
    nu = freqlo + 30.72/2
...
t.format = "ymdhms"
t = t.value
ax.set_xlabel(f"Day since first pulse (at {t[0]}-Jun-{t[2]:02d} {t[3]}:{t[4]}:{t[5]:02.0f})")
...
# Add the non-detections
# so far includes 8 days monitoring
# 2.5 hours per day
# expect 8 * 2.5 * 3 pulses
# actually detected len(files)

nzeros = int(8 * 2.5 * 3 - len(files))
limits = 0.1 * np.ones(nzeros)

fig = plt.figure(figsize=(7*cm,7*cm))
ax = fig.add_subplot(111)
ax.hist(np.concatenate([peaks, trunc_peaks, limits]))
ax.set_yscale('log')
ax.set_xlabel("peak flux density (Jy)")
ax.set_ylabel("counts")
fig.savefig("brightness_hist.png", bbox_inches="tight")
